...
I experimented with:
- randomly placing trees within grid cells
- random colour variations
- Using different svg icons for trees
...
random_points_in_grid_cells <- st_sample(grids_UK, size = rep(1, nrow(grids_UK)))
...
plot_df <- point_coord %>% 
  mutate(tree_type = sample.int(4, nrow(point_coord), replace=TRUE),
         tree_colour = sample(pallette, nrow(point_coord), replace=TRUE), 
         tree_svg = case_when(
